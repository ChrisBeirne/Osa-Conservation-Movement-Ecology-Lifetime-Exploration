...
tmp_vulture <- tmp_vulture[tmp_vulture$name !="Colonche",] 

# remove pepper peru
tmp_vulture <- tmp_vulture[!(tmp_vulture$name=="Pepper" & tmp_vulture$country=="ecuador"),]


row.names(tmp_vulture) <- 1:nrow(tmp_vulture)
nrow(tmp_vulture)
...
m1$taxon_canonical_name <- tmp_vulture$taxon_canonical_name[i] 
m1$country <- tmp_vulture$country[i]
...
The 'sticking'outlier' point is real - the sensor was only reporting once per day and was on its last legs. 
...
m %>% addPolylines(data = dat_lines,
    ...
    addCircleMarkers(lng=m_df$location_long,
                   ...
                   color="#FF10F000",
                   popup=m_df$timestamp, radius=0.2) %>% 
...
# Check the random point - it is plausible
#test <- m_df[m_df$timestamp<ymd_hms("2024-02-18 21:00:49")+days(10) & m_df$timestamp>ymd_hms("2024-02-18 21:00:49")-days(3) ,]
...
# move2
m2 = mt_as_move2(m1, track_id_columna = tmp_vulture[i])
...
tmp_red <- m2[OUT$speed<50,]
tmp_red <- tmp_red[substr(tmp_red$timestamp,1,19) != "2023-12-08 15:05:05",]

# Delete extreme values 
clean_vulture[[i]] <- tmp_red
...
m %>% addPolylines(data = dat_lines,
    ...
    addCircleMarkers(lng=m_df$location_long[nrow(m_df)],
      ...
      addCircleMarkers(lng=m_df$location_long,
                   lat=m_df$location_lat, 
                   color="#FF10F000",
                   popup=as.character(m_df$location_long)) %>% 
    # Add a legend
  addLayersControl(
    baseGroups = c("Satellite","OS", "Simple")) %>% 
  addScaleBar(position = "topleft")
...
m_df <- atl_filter_bounds(m_df, x = "location_long", 
                              y = "location_lat", x_range = c(-88, -60), 
                              y_range = c(-20, 20),remove_inside = FALSE)
# Remove the ocean location
m_df <- m_df[m_df$location_long!=-83.24109,]
...
m_df <- m_df[m_df$location_long!=m_df$location_long[2358],]
...
m1 <- mov_dat[[tmp_vulture$name[i]]] m1$taxon_canonical_name <- tmp_vulture$taxon_canonical_name[i] m1$country <- tmp_vulture$country[i]
