...
clean_vulture[[i]] <- mov_dat[[tmp_vulture$name[i]]][OUT$speed<50]

# Check the outlier is gone [Only if required]

# m2 = mt_as_move2(clean_mov[[i]], track_id_columna = tmp_vulturename[i])
# dat_lines <- m2 %>% 
#   mt_track_lines()
# m <- leaflet() %>%
#   # Add a satellite image layer
...
m2 = mt_as_move2(clean_vulture[[i]], track_id_columna = tmp_vulturename[i])
dat_lines <- m2 %>%
  mt_track_lines()
m <- leaflet() %>%
  # Add a satellite image layer
...
dir.create("tmp")
# Create.csv
vulture_stack <- moveStack(clean_vulture)
tmp_csv <- rbind(vulture_stack)
tmp_csv <- as.data.frame(tmp_csv)
write.csv(tmp_csv, "tmp/vulture_clean.csv")
saveRDS(mov_dat, "tmp/vulture_stack.RDS")
