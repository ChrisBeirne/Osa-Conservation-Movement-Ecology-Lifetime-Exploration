...
#all_summary[row.names(all_summary) %in% unique(ec_shp$local_identifier),]$country <- "Ecuador"
#all_summary[row.names(all_summary) %in% unique(peru_shp$local_identifier),]$country <- "Peru"
#all_summary[row.names(all_summary) %in% unique(cr_shp$local_identifier),]$country <- "Costa Rica"

all_summary <- all_summary[order(all_summary$taxon_canonical_name,  all_summary$days, decreasing = T),]
...
all_summary <- all_summary[,c("taxon_canonical_name", "local_identifier", "days", "timestamp_start",	"timestamp_end",	"number_of_events", "distance_moved_km",	"avg_daily_dist_km")]
```
...
#### Tags still functioning by species

Any tag reporting in the last thirty days is classed as working. 


```{r  s7}
tmp <- table(all_summary$taxon_canonical_name, (interval(ymd_hms(paste(all_summary$timestamp_end, "00:00:00")), Sys.time()) %/% days(1))<30)

tmp <- as.data.frame(tmp)
colnames(tmp) <- c("species", "functioning", "freq")
d1 <- tmp[tmp$functioning==TRUE,]
d1$functioning <- NULL
colnames(d1) <- c("species","working")
