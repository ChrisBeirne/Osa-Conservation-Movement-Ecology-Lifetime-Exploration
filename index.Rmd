...
# ## Find Google Drive folder 'Centre Circle Data & Info'
 data_folder <- drive_ls(path = "files_for_github")
# data_path <- "data" 
# dir.create(data_path) 

# Get the habitat raster 
drive_download(as_id("1siCKr305m8ca9VLQV-2mThIAYAWkrlub"))
...
```{r}
# Density raster
dat_sf <- st_as_sf(tmp_cr, coords=c("location_long", "location_lat"), crs=4326)
...
grd <- st_make_grid(dat_sf, n = 200)
# visualize the grid
plot(grd)

index <- which(lengths(st_intersects(grd, dat_sf)) > 0)
...
test <- fishnet %>%   
  ...
  st_join(dat_sf) %>% # join the species dataset
