...
mammal_dat <- moveStack(tmp, tmp2)

table(tmp$local_identifier)
#res <- distance(mammal_dat)
#sapply(res, sum)/1000  
...
{
  tmp <- interpolateTime(mammal_dat[[as.character(mammals$name[i])]], time=as.difftime(60,units="mins"), spaceMethod='greatcircle')
  tmp$distance_km <- c(NA, distance(tmp)/1000)
...
mammal_df %>% group_by(local_identifier) %>% summarize(obs_days=n()/24, dist_moved_km=sum(distance_km, na.rm=T))
